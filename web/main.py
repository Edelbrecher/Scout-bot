...
async def bot_settings_page(request: Request, guild_id: str):
    ...
    if guild.get("workspace_type") == "personal":
        return RedirectResponse(f"/guild/{guild_id}", status_code=302)

    qs = f"?{request.url.query}" if request.url.query else ""
    return RedirectResponse(f"/guild/{guild_id}/my-ally{qs}#bot-settings", status_code=302)
...
async def bot_settings_language(request: Request, guild_id: str, bot_language: str = Form("de")):
    ...
    return RedirectResponse(f"/guild/{guild_id}/my-ally?saved=language#bot-settings", status_code=303)
...
async def my_ally_page(request: Request, guild_id: str):
    ...
    if is_lead:
        ...
        request_hub = await database.get_request_hub(guild_id)
    bot_settings_saved = request.query_params.get("saved", "")
    bot_settings_error = request.query_params.get("error", "")
...
def _build_countdown_embed(end_date_str: str, ally_name: str, utc_offset_minutes: int = 60, button_tooltip: str | None = None) -> tuple[dict, int, int, int]:
    ...
    tz_hours  = abs(utc_offset_minutes) // 60
    tz_label  = f"UTC{sign}{tz_hours}" if tz_hours else "UTC"
    end_str   = end_dt.astimezone(local_tz).strftime(f"%d.%m.%Y %H:%M {tz_label}")
...
_DEFAULT_SIDEBAR_NAV = [
    ...
    {"type": "item",  "icon": "map",       "label": "Map",                "url_suffix": "/map", "aliases": "karte"},
    {"type": "item",  "icon": "radar",     "label": "Sector Monitor",     "url_suffix": "/map/sector-monitor", "aliases": "sektor"},
    {"type": "item",  "icon": "search",    "label": "Player Search",      "url_suffix": "/intel", "aliases": "spieler suche"},
    {"type": "item",  "icon": "eye",       "label": "Scout Tracking",     "url_suffix": "/scout", "aliases": "späher kundschafter"},
    ...
    {"type": "item",  "icon": "shield",    "label": "Defense",            "url_suffix": "/verteidigung", "aliases": "verteidigung deff def"},
    {"type": "item",  "icon": "scroll",    "label": "Battle Reports",     "url_suffix": "/reports", "aliases": "berichte kampfberichte"},
    # ── Farming ───────────────────────────────────────────────────────────
    ...
    {"type": "item",  "icon": "users",     "label": "Members",            "url_suffix": "/allianz/mitglieder", "aliases": "mitglieder"},
    {"type": "item",  "icon": "skull",     "label": "Enemies",            "url_suffix": "/enemies", "aliases": "gegner feinde"},
    ...
    {"type": "item",  "icon": "diamond",   "label": "Artifacts",          "url_suffix": "/artifacts", "aliases": "artefakt artefakte arte rotation"},
    {"type": "item",  "icon": "castle",    "label": "Treasuries",         "url_suffix": "/all-treasuries", "aliases": "schatzkammer schatz treasury"},
    {"type": "item",  "icon": "map",       "label": "Artifact Spawn Map", "url_suffix": "/artifacts/spawn-map", "aliases": "artefakt spawn karte"},
    # ── Tools ─────────────────────────────────────────────────────────────
    {"type": "group", "label": "Tools"},
    {"type": "item",  "icon": "box",       "label": "Res Push",           "url_suffix": "/res-push"},
    {"type": "item",  "icon": "chart",     "label": "Statistics",         "url_suffix": "/stats", "aliases": "statistik"},
    ...
    {"type": "item",  "icon": "gear",      "label": "Settings",           "url_suffix": "/settings", "aliases": "einstellungen"},
...
async def create_request_hub(request: Request, guild_id: str):
    ...
    try:
        async with httpx.AsyncClient(timeout=15) as client:
            ...
            data = resp.json()
            if data.get("ok"):
                return RedirectResponse(f"/guild/{guild_id}/my-ally?saved=hub_created#bot-settings", status_code=303)
            print(f"[hub_create] bot error: {data}")
            return RedirectResponse(f"/guild/{guild_id}/my-ally?error=hub_create_failed&detail={data.get('error','')}#bot-settings", status_code=303)
    except Exception as e:
        print(f"[hub_create] exception: {type(e).__name__}: {e}")
        return RedirectResponse(f"/guild/{guild_id}/my-ally?error=hub_create_failed#bot-settings", status_code=303)
...
async def refresh_request_hub(request: Request, guild_id: str):
    ...
    try:
        async with httpx.AsyncClient(timeout=15) as client:
            ...
            data = resp.json()
            if data.get("ok"):
                return RedirectResponse(f"/guild/{guild_id}/my-ally?saved=hub_refreshed#bot-settings", status_code=303)
            return RedirectResponse(f"/guild/{guild_id}/my-ally?error=hub_refresh_failed&detail={data.get('error','')}#bot-settings", status_code=303)
    except Exception as e:
        return RedirectResponse(f"/guild/{guild_id}/my-ally?error=hub_refresh_failed#bot-settings", status_code=303)
...
async def digest_set(request: Request, guild_id: str):
    ...
    if not (_is_leader(session, guild_id) or session.get("type") == "admin"):
        return RedirectResponse(f"/guild/{guild_id}/my-ally#bot-settings", status_code=303)
    form = await request.form()
    ...
    return RedirectResponse(f"/guild/{guild_id}/my-ally?saved=digest#bot-settings", status_code=303)
...
async def digest_clear(request: Request, guild_id: str):
    ...
    return RedirectResponse(f"/guild/{guild_id}/my-ally?saved=digest_cleared#bot-settings", status_code=303)
...
async def alliance_bonuses_page(request: Request, guild_id: str):
    ...
    enabled_keys = await database.get_bonus_enabled_keys(ally_group["id"], all_keys)
    ...
    # Filter out disabled keys from order, build default if empty
    research_order = [s for s in research_order if s["key"] in enabled_keys]
...
@app.patch("/guild/{guild_id}/my-ally/bonuses/enabled-keys")
async def alliance_bonuses_save_enabled(request: Request, guild_id: str):
    ...
    if not is_editor:
        return JSONResponse({"error": "forbidden"}, status_code=403)
    body = await request.json()
    valid = {b["key"] for b in ALLIANCE_BONUS_DEFS}
    keys = [k for k in body.get("keys", []) if k in valid]
    await database.save_bonus_enabled_keys(ally_group["id"], keys)
    # Rebuild research order without disabled keys
